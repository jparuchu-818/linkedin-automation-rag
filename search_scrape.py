...
BLOCKED_DOMAINS = ['reddit.com', 'facebook.com', 'twitter.com', 'instagram.com', 'imdb.com']
...
def get_ngrok_public_url():
    try:
        ...
        if tunnels:
            ...
            if https_tunnel: return https_tunnel.get("public_url")
        return None
    except requests.exceptions.ConnectionError:
        print("[ERROR] Could not connect to ngrok agent. Is 'run_approval_server.py' running?")
        return None
...
def process_one_topic():
    ...
    try:
        ...
        if not public_url:
            raise ConnectionError("Could not get ngrok URL. Ensure approval server is running.")

        # Store post with 'pending_approval' status
        post_id = store_post({
            "topic": topic, "generated_post": post_text,
            "generated_image_path": image_path, "status": "pending_approval",
        ...
        send_approval_email(
            ...
            post_text=post_text, image_path=image_path,
        ...
        print(f"[WORKFLOW ERROR] A failure occurred while processing topic '{topic}': {e}")
